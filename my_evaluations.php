...
if (tbl_exists($pdo,'evaluations')) {
  ...
  $dateC  = pick_col($pdo,'evaluations',['due_date','review_date','next_review_at','created_at']);

  $empId = null;
  if (tbl_exists($pdo,'employees') && col_exists($pdo,'employees','user_id')) {
    $q=$pdo->prepare("SELECT id FROM employees WHERE user_id=? LIMIT 1");
    $q->execute([(int)($u['id'] ?? 0)]); $empId = (int)$q->fetchColumn();
  }
  $where = "1=1"; $pr=[];
  if ($empKey && $empId) { $where="ev.`$empKey`=?"; $pr[]=$empId; }
  elseif ($empKey)        { $where="ev.`$empKey`=?"; $pr[]=(int)($u['id']??0); }

  $sql="SELECT ".($perC?"ev.`$perC`":"NULL")." AS period,
               ".($statC?"ev.`$statC`":"NULL")." AS status,
               ".($scoreC?"ev.`$scoreC`":"NULL")." AS score,
               ".($dateC?"ev.`$dateC`":"NULL")." AS dt
        FROM evaluations ev
        WHERE $where
        ORDER BY COALESCE(ev.`$dateC`, ev.id) DESC";
  try{ $st=$pdo->prepare($sql); $st->execute($pr); $rows=$st->fetchAll(PDO::FETCH_ASSOC)?:[]; }catch(Throwable $e){}
}
...
<meta charset="UTF-8"><title>My Evaluations | <?=htmlspecialchars($brandName)?></title>
...
<header class="sticky top-0 z-40">
  <div id="topbarPad" class="ml-64 bg-white/90 backdrop-blur border-b border-slate-200">
    <div class="h-14 px-3 md:px-4 flex items-center gap-3">
      ...
      <div class="text-slate-700 font-medium">My Evaluations</div>
    ...
    <div class="px-6 py-6">
      <div class="bg-white ring-1 ring-slate-200 rounded-xl overflow-hidden">
        <table class="w-full text-sm">
          ...
          <tr><th class="px-4 py-3 text-left">Period</th><th class="px-4 py-3 text-left">Status</th><th class="px-4 py-3 text-right">Score</th><th class="px-4 py-3 text-left">Date</th></tr>
          </thead>
          <tbody>
          <?php if ($rows): foreach($rows as $r): ?>
            <tr class="border-t">
              <td class="px-4 py-3"><?= htmlspecialchars($r['period']??'—') ?></td>
              <td class="px-4 py-3"><?= htmlspecialchars(ucfirst(strtolower($r['status']??''))) ?></td>
              <td class="px-4 py-3 text-right"><?= ($r['score']!==null && $r['score']!=='') ? htmlspecialchars((string)$r['score']) : '—' ?></td>
              <td class="px-4 py-3"><?= !empty($r['dt']) ? date('M d, Y', strtotime($r['dt'])) : '—' ?></td>
            </tr>
          <?php endforeach; else: ?>
            <tr><td colspan="4" class="px-4 py-8 text-center text-slate-500">No performance records.</td></tr>
          <?php endif; ?>
          </tbody>
        </table>
      </div>
